...
# changes by me
from torchvision.ops import roi_pool as RoIPoolFunction
...
# changes by me
    image = imresize(image[0], tuple(self._im_info[0][1::-1] / self._im_info[0][2])) # assume we only have 1 image
...
# changes by me
    ...
    rpn_cls_score_reshape = rpn_cls_score.contiguous().view(self._batch_size, 2, -1, rpn_cls_score.size()[-1]) # batch * 2 * (num_anchors*h) * w
...
# changes by me
      summaries.append(tb.summary.scalar(key, var.data))
...
#       for k in d.keys():
  ...
  def train_step(self, blobs, train_op):
    self.forward(blobs['data'], blobs['im_info'], blobs['gt_boxes'], blobs['gt_masks'])
# changes by me
    rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_mask, loss = self._losses["rpn_cross_entropy"].data, \
                                                                        self._losses['rpn_loss_box'].data, \
                                                                        self._losses['cross_entropy'].data, \
                                                                        self._losses['loss_box'].data, \
                                                                        self._losses['loss_mask'].data, \
                                                                        self._losses['total_loss'].data
